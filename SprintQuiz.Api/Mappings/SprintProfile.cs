...
namespace SprintQuiz.Api.Mappings
{
    public class SprintProfile : Profile
    {
        ...
        {
            // Mapping Sprint → SprintDto
            CreateMap<Sprint, SprintDto>()
                .ForMember(dest => dest.Modules, opt => opt.MapFrom(src => src.Modules))
                .ForMember(dest => dest.DerniereActivite, opt => opt.Ignore()); // Calculé dynamiquement dans le service

            // Mapping CreateSprintDto → Sprint
            ...
            // Mapping UpdateSprintDto → Sprint 
